...
type ViewerOpportunityPanelProps = {
  opportunity: ReturnType<typeof mapOpportunity>;
  userId?: string;
  requiresCoachManagedTunnelTime: boolean;
};
    ...
    <>
      ...
      <div className="grid gap-3 lg:grid-cols-[1fr_300px]">
        <article className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:p-5">
          ...
          <Suspense fallback={<ViewerOpportunityFallback />}>
            <ViewerOpportunityPanel
              opportunity={opportunity}
              userId={user?.id}
              requiresCoachManagedTunnelTime={requiresCoachManagedTunnelTime}
            />
          </Suspense>
...
async function ViewerOpportunityPanel({
  opportunity,
  userId,
  requiresCoachManagedTunnelTime,
}: ViewerOpportunityPanelProps) {
  const supabase = await createSupabaseServerClient();
  const isHuckJam = opportunity.type === "huck_jam";
  const isCamp = opportunity.type === "camp";
  const followTargets = userId
    ? [
        ...
        (
          target,
        ): target is { target_type: "tunnel" | "coach"; target_id: string } =>
          Boolean(target),
      )
    : [];
  const [
  ...
  const viewerSelfBookingEnabled = viewerInterest?.self_booking_enabled === true;
  ...
  const bookedTimes = ((bookingResult.data ?? []) as BookingRow[])
    .map((booking) => {
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {opportunity.coachFollowId ? (
            <FollowButton
              ...
              label={opportunity.type === "camp" && opportunity.coachName ? "Follow Coach" : "Follow Organizer"}
...
function ViewerOpportunityFallback() {
  return (
    <div className="mt-4 rounded-2xl border border-sky-100 bg-sky-50 p-4">
      <div className="h-4 w-24 animate-pulse rounded bg-sky-100" />
      <div className="mt-2 h-5 w-3/4 animate-pulse rounded bg-sky-100" />
