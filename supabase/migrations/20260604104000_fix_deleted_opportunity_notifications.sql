...
begin
  ...
  where id = target_opportunity_id::uuid
  for update;
  ...
  select distinct
    affected_user_id,
    opportunity_kind || ' was removed',
    coalesce(opportunity_record.title, opportunity_kind) || ' was deleted by the organizer.',
    'opportunity_deleted',
    null::uuid
  from (
    select oi.athlete_id as affected_user_id
    from public.opportunity_interests oi
    where oi.opportunity_id = target_opportunity_id::uuid

    union

    select osb.user_id as affected_user_id
    from public.opportunity_slot_bookings osb
    where osb.opportunity_id = target_opportunity_id::uuid
  ) affected
  ...
  if exists (
    ...
    execute $delete_recipients$
      ...
      where nr.notification_id = n.id
        and n.opportunity_id = $1::uuid
    $delete_recipients$
    using target_opportunity_id::uuid;
  end if;
  ...
  if exists (
    ...
    execute 'delete from public.applications where opportunity_id = $1::uuid'
    using target_opportunity_id::uuid;
  end if;
  ...
  for dependent_fk in
    ...
    where constraint_info.contype = 'f'
      ...
      and child_column.atttypid = 'uuid'::regtype
      and array_length(constraint_info.conkey, 1) = 1
  loop
    execute format(
      'delete from %I.%I where %I = $1::uuid',
